...
class DashboardAnalyticsViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        ...
        # ── VIEW 2: RECEPTIONIST ──────────────────────────────────────────
            ...
            active_members_count = ClientSubscription.objects.filter(
                is_active=True
            ).count()
        ...
        # ── VIEW 3: ADMIN ─────────────────────────────────────────────────
            ...
            for trainer_id, total_lost in deductions_qs:
                if trainer_id is not None:
                    adjustments[trainer_id] = adjustments.get(
                        trainer_id, Decimal(0)
                    ) - (total_lost or Decimal(0))
            for trainer_id, total_gained in additions_qs:
                if trainer_id is not None:
                    adjustments[trainer_id] = adjustments.get(
                        trainer_id, Decimal(0)
                    ) + (total_gained or Decimal(0))

            group_adj = _compute_group_adjustments(month, year)
            for trainer_id, amount in group_adj.items():
                adjustments[trainer_id] = (
                    adjustments.get(trainer_id, Decimal(0)) + amount
                )
...
class GroupTrainingViewSet(viewsets.ModelViewSet):
                    ...
                    child_performance.append(
                        {
                            ...
                            or _legacy_type_to_category(ex.get("type", "")),
    ...
    def bulk_exercise_history(self, request):
        ...
        # 1. إنشاء قاموس يربط الاسم (بحروف صغيرة) بالاسم الأصلي القادم من الـ Frontend
        ...
        for p in participations:
            ...
            # لو لقينا تاريخ لكل التمارين المطلوبة لهذا المتدرب، نتخطى اللوب
            ...
            for ex in exercises_data if isinstance(exercises_data, list) else []:
                ...
                # نتجاهل التمرين لو مش من ضمن التمارين المطلوبة
                if ex_name_lower not in ex_name_set:
                    continue

                # 3. إرجاع الاسم لشكله الأصلي الذي طلبه الـ Frontend
                original_requested_name = requested_names_map[ex_name_lower]

                # لو كنا لقينا نتيجة سابقة للتمرين ده للمتدرب ده، نتخطاه (عشان إحنا مرتبين من الأحدث للأقدم)
                ...
                if user_res:
                    # 4. حفظ النتيجة باستخدام المفتاح الأصلي
        ...
        # 5. أي تمرين ملوش بيانات سابقة، نضع له found: False
    ...
    def complete_session(self, request):
        """
        FIX B1: Atomically creates a GroupSessionLog and all associated
        GroupSessionParticipant records in a single transaction.

        Expected payload:
          {
            "day_name":          "Monday",
            "exercises_summary": [ { "name": ..., "category": ..., "sets_count": ..., "results": [...] } ],
            "participants":      [ { "client_id": 42, "note": "Completed" } ]
          }

        Setting deducted=True on each participant triggers the model's save()
        signal which atomically increments sessions_used on the client's active
        ClientSubscription and auto-deactivates if the plan limit is reached.
        """
        ...
        with transaction.atomic():
            ...
            for p in participants_data:
                ...
                GroupSessionParticipant.objects.create(
                    ...
                    # Setting deducted=True triggers the model.save() hook which
                    # atomically increments sessions_used on the client's active
                    # subscription and auto-deactivates if plan limit is hit.
                    deducted=True,
                )
...
class SessionTransferRequestViewSet(viewsets.ModelViewSet):
    ...
    def respond(self, request, pk=None):
        ...
        if transfer.to_trainer != request.user:
            return Response(
                {"error": "Not authorized to respond to this request."}, status=403
            )
        ...
        with transaction.atomic():
            ...
            remaining = (
                (plan.units - source_sub.sessions_used) if plan and plan.units else 0
            )
...
class TrainerShiftViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=False, methods=["get", "put", "patch"], url_path="mine", url_name="mine"
    )
...
class AdminTrainerOversightViewSet(viewsets.ViewSet):
    ...
    def _build_mini_dashboard_data(self, trainer, request):
        ...
        for grp in group_sessions:
            d = grp.date.date()
            participant_names = [
                p.client.name for p in grp.participants.all() if p.client
            ]
            session_activity.append(
                {
                    "type": "group",
                    "date": d,
                    "client_name": (
                        ", ".join(participant_names) if participant_names else "Group"
                    ),
