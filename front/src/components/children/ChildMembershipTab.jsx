                                ...
                                <div 
                                    ...
                                    className={`p-4 rounded-2xl border cursor-pointer transition-all relative group
                                        ${selectedSub?.id === sub.id 
                                            ? 'bg-gradient-to-br from-blue-500/20 to-zinc-900 border-blue-500/50 shadow-lg shadow-blue-900/20' 
                                            : 'bg-zinc-900/50 border-zinc-800 hover:bg-zinc-800 hover:border-zinc-700'}
                                        ${!sub.is_active && 'opacity-60 grayscale-[0.5]'}
                                    `}
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-base text-white truncate pr-2">{sub.plan_name || 'Child Plan'}</h4>
                    ...
                    <div className="flex justify-between items-start mb-8 border-b border-zinc-800/50 pb-6">
                        <div>
                            <h3 className="text-2xl font-black flex items-center gap-3 text-white">
                                <span className="bg-blue-500/10 p-2 rounded-xl text-blue-500"><Activity size={24} /></span>
                                Growth & Stats
                            </h3>
                        ...
                        <button 
                            onClick={onSaveClick} 
                            disabled={isSaving}
                            className="flex items-center gap-2 bg-white text-black hover:bg-blue-500 hover:text-white px-6 py-3 rounded-xl font-bold text-sm transition-all shadow-lg shadow-white/5 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {isSaving ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-zinc-300 uppercase tracking-wider ml-1 flex items-center gap-1">
                                <Ruler size={12} className="text-blue-500"/> Height (cm)
                            </label>
                            <input 
                                type="number" 
                                value={selectedSub.inbody_height || ''} 
                                onChange={(e) => setSelectedSub({...selectedSub, inbody_height: e.target.value})} 
                                className="w-full bg-black/40 border border-zinc-700/50 focus:border-blue-500 focus:bg-zinc-900 rounded-xl px-4 py-3 text-white font-mono text-lg transition-all outline-none" 
                            />
                        </div>
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-zinc-300 uppercase tracking-wider ml-1 flex items-center gap-1">
                                <Target size={12} className="text-blue-500"/> Muscle (kg)
                            </label>
                            <input 
                                type="number" 
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-zinc-300 uppercase tracking-wider ml-1 flex items-center gap-1">
                                <Activity size={12} className="text-blue-500"/> Body Fat (%)
                            </label>
                            <input 
                                type="number" 
                        ...
                        <div className="lg:col-span-2 space-y-2">
                            <label className="text-xs font-bold text-zinc-300 uppercase tracking-wider ml-1">Target</label>
                        ...
                        <div className="md:col-span-2 lg:col-span-4 space-y-2">
                            <label className="text-xs font-bold text-zinc-300 uppercase tracking-wider ml-1">Notes / Progress</label>
                            <textarea 
                                rows="3" 
                                value={selectedSub.inbody_notes || ''} 
                                onChange={(e) => setSelectedSub({...selectedSub, inbody_notes: e.target.value})} 
                                className="w-full bg-black/40 border border-zinc-700/50 focus:border-blue-500 rounded-xl px-4 py-3 text-white outline-none resize-none" 
