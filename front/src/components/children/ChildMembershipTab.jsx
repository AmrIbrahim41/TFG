...
import {
    ...
    Target, Activity, Waves, Dumbbell, ArrowRight, Save, Loader2, Baby,
...
// --- STATS INPUT FIELD ---
const StatInput = ({ label, icon: Icon, value, onChange, suffix }) => (
    <div className="space-y-2">
        <label className="text-xs font-bold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider ml-1 flex items-center gap-1">
            <Icon size={12} className="text-blue-600 dark:text-blue-500" /> {label}
        </label>
...
const ChildMembershipTab = ({
    ...
    // Computed water percentage — memoized so it doesn't recalculate on every render
    ...
    const onSaveClick = useCallback(async () => {
        ...
        try {
            ...
            setSaveError('Failed to save stats. Please try again.');
        } finally {
            setIsSaving(false);
        }
    }, [handleSaveInBody, selectedSub]);

    // Helper to update a single inbody field without re-creating the whole object reference each keystroke
                ...
                <div className="flex justify-between items-center">
                    ...
                    {!hasActiveSub && (
                        <button
                            ...
                            className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-blue-500 transition-colors shadow-lg shadow-blue-900/20"
                        >
                ...
                <div className="min-h-[200px]">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        {subscriptions.length === 0 && (
                            <div className="col-span-full text-zinc-500 text-center py-12 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-2xl flex flex-col items-center gap-3">
                                <Baby size={32} className="opacity-20" />
                                <p className="font-bold text-sm">No memberships assigned yet</p>
                                <button
                                    onClick={() => setIsSubModalOpen(true)}
                                    className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-blue-500 transition-colors text-sm"
                                >
                                    <Plus size={14} /> Assign First Plan
                                </button>
                            </div>
                        )}
                    </div>
            ...
            {selectedSub && (
                <div className="bg-zinc-50 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 md:p-8 animate-in slide-in-from-bottom-5 fade-in duration-300 shadow-2xl">
                    ...
                    <div className="flex justify-between items-start mb-8 border-b border-zinc-300 dark:border-zinc-800/50 pb-6">
                        <div>
                            ...
                            <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1 ml-1">
                                Plan: <span className="text-zinc-900 dark:text-white font-bold">{selectedSub.plan_name || 'Custom'}</span>
                            </p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <StatInput
                            label="Height (cm)" icon={Ruler}
                            value={selectedSub.inbody_height}
                            onChange={(e) => updateField('inbody_height', e.target.value)}
                        />
                        <StatInput
                            label="Weight (kg)" icon={Scale}
                            value={selectedSub.inbody_weight}
                            onChange={(e) => updateField('inbody_weight', e.target.value)}
                        />
                        <StatInput
                            label="Muscle (kg)" icon={Target}
                            value={selectedSub.inbody_muscle}
                            onChange={(e) => updateField('inbody_muscle', e.target.value)}
                        />
                        <StatInput
                            label="Body Fat (%)" icon={Activity}
                            value={selectedSub.inbody_fat}
                            onChange={(e) => updateField('inbody_fat', e.target.value)}
                        />
                        ...
                        <div className="md:col-span-2 lg:col-span-4 space-y-2">
                            ...
                            <textarea
                                ...
                                className="w-full bg-zinc-200 dark:bg-black/40 border border-zinc-300 dark:border-zinc-700/50 focus:border-blue-500 rounded-xl px-4 py-3 text-zinc-900 dark:text-white outline-none resize-none transition-all"
