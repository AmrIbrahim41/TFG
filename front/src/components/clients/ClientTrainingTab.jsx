...
const ClientTrainingTab = ({ subscriptions }) => {
    ...
    const renderProgressBar = () => {
        ...
        return (
            <div className="bg-[#121214] border border-zinc-800 rounded-2xl p-4 md:p-6 mb-6 relative overflow-hidden">
                <div className="relative z-10 flex justify-between items-end mb-3">
                    <div>
                        <h4 className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">Progress</h4>
                    ...
                    <Trophy size={24} className={percentage === 100 ? "text-yellow-500" : "text-zinc-700"} />
                </div>
                ...
                {cycles.map((cycle, cIndex) => (
                    <div key={cIndex} className="space-y-4">
                        ...
                        <div className="flex items-center gap-3 px-1">
                            ...
                            <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest bg-[#09090b] px-3 py-1 rounded-full border border-zinc-800">
                                Cycle {cIndex + 1}
                            </span>
                            <div className="h-px flex-1 bg-zinc-800" />
                        </div>
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`
                                            ...
                                            ${isCompleted 
                                                ? 'bg-[#121214] border-emerald-500/30 hover:bg-emerald-500/5 hover:border-emerald-500/50' 
                                                : 'bg-[#121214] border-zinc-800 border-dashed hover:border-solid hover:border-orange-500/50 hover:bg-zinc-900 hover:-translate-y-1 hover:shadow-xl hover:shadow-orange-900/10'
                                            }
                                        `}
                                    >
                                        ...
                                        <div className="flex justify-between items-start p-4 pb-2">
                                            ...
                                            {isCompleted ? (
                                                <div className="bg-emerald-500 rounded-full p-1 shadow-lg shadow-emerald-500/20">
                                                    <CheckCircle size={12} className="text-black fill-current" />
                                                </div>
                                            ) : (
                                                <CircleDashed size={14} className="text-zinc-600 group-hover:text-orange-500 transition-colors" />
                                            )}
                                        </div>
                                        ...
                                        {isCompleted ? (
                                            ...
                                            <div className="mt-3 px-4 py-3 border-t border-zinc-800/50 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                                                <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wider">Start Workout</span>
                        ...
                        {displayedSubs.map((sub) => (
                            <div 
                                ...
                                className={`
                                    ...
                                    ${sub.is_active 
                                        ? 'bg-[#121214] border-zinc-800 hover:border-orange-500 hover:shadow-xl hover:shadow-orange-900/10' 
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {trainingPlan && (
                            <button 
                                ...
                                className="p-2 bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-red-500 hover:border-red-500/50 rounded-lg transition-all"
                                title="Reset Plan"
                            >
                                <Trash2 size={16} />
                            </button>
                    ...
                    {loadingPlan ? (
                        ...
                        <div className="max-w-md mx-auto py-8 animate-in zoom-in-95 duration-300">
                            {setupStep === 1 && (
                                <div className="bg-[#121214] border border-zinc-800 rounded-3xl p-8 text-center shadow-2xl relative overflow-hidden">
                                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-500 to-purple-500" />
                                    
                                    <div className="w-16 h-16 bg-zinc-900 rounded-2xl border border-zinc-800 flex items-center justify-center mx-auto mb-6 text-white shadow-inner">
                                        <RefreshCw size={32} />
                                    </div>
                                    
                                    <h3 className="text-2xl font-black text-white mb-2">Split Structure</h3>
                                    <p className="text-zinc-500 text-sm mb-8">How many different workouts rotate in this cycle?</p>
                                    
                                    {/* Counter UI */}
                                    <div className="flex items-center justify-center gap-6 mb-8">
                                        <button onClick={handleDecrement} className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-white text-white flex items-center justify-center transition-all active:scale-90">
                                            <Minus size={20} />
                                        </button>
                                        <div className="w-20 text-center">
                                            <span className="text-4xl font-black text-white">{cycleLength}</span>
                                            <p className="text-[10px] uppercase font-bold text-zinc-600 mt-1">Days</p>
                                        </div>
                                        <button onClick={handleIncrement} className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-white text-white flex items-center justify-center transition-all active:scale-90">
                                            <Plus size={20} />
                                        </button>
                                    </div>

                                    <button onClick={handleNextStep} className="w-full bg-white text-black hover:bg-orange-500 hover:text-white font-bold py-4 rounded-xl transition-all shadow-lg active:scale-95">
                                        Continue
                                    </button>
                                </div>
                            )}
                            ...
                            {setupStep === 2 && (
                                <div className="bg-[#121214] border border-zinc-800 rounded-3xl p-8 shadow-2xl animate-in slide-in-from-right-8 duration-300">
                                    <div className="flex items-center gap-3 mb-6">
                                        <button onClick={() => setSetupStep(1)} className="p-2 hover:bg-zinc-900 rounded-lg text-zinc-500"><ArrowLeft size={18}/></button>
                                        <h3 className="text-xl font-bold text-white">Name Your Days</h3>
                                    </div>

                                    <div className="space-y-3 mb-8 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                        {Array.from({ length: cycleLength }).map((_, i) => (
                                            <div key={i} className="flex items-center gap-3 group">
                                                <div className="w-8 h-8 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500 font-bold text-xs group-focus-within:border-orange-500 group-focus-within:text-orange-500 transition-colors">
                                                    {i + 1}
                                                </div>
                                                <input 
                                                    type="text" 
                                                    ...
                                                    value={dayNames[i + 1] || ''} 
                                                    className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-500 transition-colors text-sm" 
