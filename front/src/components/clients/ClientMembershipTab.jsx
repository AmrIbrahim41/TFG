...
import {
    Plus, Calendar, Scale, Power, UserCheck, Ruler, Droplets,
    Target, Activity, Waves, Dumbbell, ArrowRight, Save, Loader2,
                ...
                <button
                    ...
                    className={`w-8 h-8 rounded-lg font-bold text-xs transition-all border
                        ${currentPage === idx + 1
                            ...
                            : 'bg-zinc-200 dark:bg-zinc-900 border-zinc-300 dark:border-zinc-800 text-zinc-600 dark:text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-400 dark:hover:border-zinc-700'
                        }
                    `}
                >
                    {idx + 1}
...
// --- REUSABLE STAT INPUT ---
const StatInput = ({ label, icon: Icon, accentColor = 'text-orange-500', value, onChange, suffix }) => (
    <div className="space-y-2">
        ...
        <div className="relative">
            <input
                ...
                onChange={onChange}
                inputMode="decimal"
                className="w-full bg-zinc-200 dark:bg-black/40 border border-zinc-300 dark:border-zinc-700/50 focus:border-orange-500 focus:bg-zinc-100 dark:focus:bg-zinc-900 rounded-xl px-4 py-3 text-zinc-900 dark:text-white font-mono text-lg transition-all outline-none"
            />
            {suffix && (
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-500 dark:text-zinc-600 pointer-events-none">
                    {suffix}
                </span>
            )}
        </div>
...
const ClientMembershipTab = ({
    ...
    const waterPercent = useMemo(() => {
        ...
        return 0;
    }, [selectedSub?.inbody_weight, selectedSub?.inbody_tbw]);
        ...
        <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    <h3 className="text-xl font-bold text-zinc-900 dark:text-white">Subscription History</h3>
                    {!hasActiveSub && (
                        <button
                            onClick={() => setIsSubModalOpen(true)}
                            className="flex items-center gap-2 bg-zinc-900 dark:bg-white text-white dark:text-black px-4 py-2 rounded-xl font-bold hover:bg-orange-600 dark:hover:bg-zinc-200 transition-colors shadow-lg shadow-zinc-400/20 dark:shadow-white/5"
                        >
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-4 rounded-2xl border cursor-pointer transition-all relative group
                                        ${isSelected
                                            ? 'bg-gradient-to-br from-orange-500/10 to-zinc-200 dark:from-orange-500/20 dark:to-zinc-900 border-orange-500/50 shadow-lg shadow-orange-900/20'
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-base text-zinc-900 dark:text-white truncate pr-2">
                                            {sub.plan_name || 'Custom Plan'}
                                        </h4>
                                        <div className="flex items-center gap-2 shrink-0">
                                            <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-md border
                                                ${sub.is_active
                                                    ? 'bg-green-500/20 dark:bg-green-500 text-green-700 dark:text-black border-green-500'
                                                    : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-500 border-zinc-300 dark:border-zinc-700'
                                                }`}
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`p-1 rounded-md border transition-all
                                                    ${sub.is_active
                                                        ? 'text-green-600 dark:text-green-500 border-green-900/30 hover:bg-red-500 hover:text-white hover:border-red-500'
                                                        : 'text-zinc-500 dark:text-zinc-600 border-zinc-300 dark:border-zinc-700 hover:text-green-600 dark:hover:text-green-500 hover:bg-green-500/10'
                                                    }
                                                `}
                                            >
                                                <Power size={14} />
                                    ...
                                    <div className="flex flex-col gap-2 mb-3">
                                        <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-200 dark:bg-black/30 px-2 py-1.5 rounded-lg w-fit">
                                            ...
                                            <ArrowRight size={10} className="text-zinc-500 dark:text-zinc-600" />
                                    ...
                                    {sub.is_active && total > 0 && (
                                        <div className="bg-zinc-200 dark:bg-black/40 rounded-lg p-2 border border-zinc-300 dark:border-zinc-800/50 flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                <span className={`text-sm font-black ${remaining <= 3 ? 'text-red-500' : 'text-zinc-800 dark:text-white'}`}>
                                                    {remaining}
                                                </span>
                                                <span className="text-[10px] text-zinc-500 dark:text-zinc-600 font-medium">/ {total} Sessions</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
            ...
            {selectedSub && (
                <div className="bg-zinc-50 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 md:p-8 animate-in slide-in-from-bottom-5 fade-in duration-300 shadow-2xl">
                    ...
                    <div className="flex justify-between items-start mb-8 border-b border-zinc-300 dark:border-zinc-800/50 pb-6">
                        <div>
                            <h3 className="text-2xl font-black flex items-center gap-3 text-zinc-900 dark:text-white">
                                <span className="bg-orange-500/10 p-2 rounded-xl text-orange-600 dark:text-orange-500"><Scale size={24} /></span>
                                InBody Analysis
                            </h3>
                            <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1 ml-1">
                                Plan: <span className="text-zinc-800 dark:text-white font-bold">{selectedSub.plan_name || 'Custom'}</span>
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-2">
                            ...
                            <button
                                onClick={onSaveClick}
                                disabled={isSaving}
                                className="flex items-center gap-2 bg-zinc-900 dark:bg-white text-white dark:text-black hover:bg-orange-500 dark:hover:bg-orange-500 hover:text-white dark:hover:text-white px-6 py-3 rounded-xl font-bold text-sm transition-all shadow-lg shadow-zinc-400/20 dark:shadow-white/5 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isSaving ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <StatInput label="Height (cm)" icon={Ruler} value={selectedSub.inbody_height} onChange={(e) => updateField('inbody_height', e.target.value)} />
                        <StatInput label="Weight (kg)" icon={Scale} value={selectedSub.inbody_weight} onChange={(e) => updateField('inbody_weight', e.target.value)} />
                        <StatInput label="Muscle (kg)" icon={Target} value={selectedSub.inbody_muscle} onChange={(e) => updateField('inbody_muscle', e.target.value)} />
                        <StatInput label="Body Fat (%)" icon={Activity} value={selectedSub.inbody_fat} onChange={(e) => updateField('inbody_fat', e.target.value)} />
                        ...
                        <div className="lg:col-span-2 space-y-3 bg-blue-500/5 border border-blue-500/10 rounded-2xl p-4">
                            ...
                            <div className="relative h-12 bg-zinc-200 dark:bg-black/40 rounded-xl overflow-hidden border border-blue-500/20">
                                <div
                                    ...
                                    style={{ width: `${Math.min(Number(waterPercent), 100)}%`, opacity: 0.7 }}
                                >
                                    <div
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateField('inbody_tbw', e.target.value)}
                                    inputMode="decimal"
                                    ...
                                    placeholder="0.0"
                                />
                                <div className="absolute right-4 top-3 pointer-events-none text-xs font-bold text-zinc-500 dark:text-white/50">Liters</div>
                            </div>
                        </div>

                        {/* Goal Selector */}
                        <div className="lg:col-span-2 space-y-2">
                            <label className="text-xs font-bold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider ml-1">Current Goal</label>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {Object.entries(GOAL_CONFIG).map(([goal, style]) => (
                                    <button
                                        key={goal}
                                        type="button"
                                        onClick={() => updateField('inbody_goal', goal)}
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {Object.entries(ACTIVITY_CONFIG).map(([level, style]) => (
                                    <button
                                        key={level}
                                        type="button"
                                        onClick={() => updateField('inbody_activity', level)}
                        ...
                        <div className="md:col-span-2 lg:col-span-4 space-y-2">
                            <label className="text-xs font-bold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider ml-1">Analysis Notes</label>
                            <textarea
                                rows={3}
                                value={selectedSub.inbody_notes || ''}
                                onChange={(e) => updateField('inbody_notes', e.target.value)}
                                className="w-full bg-zinc-200 dark:bg-black/40 border border-zinc-300 dark:border-zinc-700/50 focus:border-orange-500 rounded-xl px-4 py-3 text-zinc-900 dark:text-white outline-none resize-none transition-all"
