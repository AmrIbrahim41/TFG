...
const theme = {
    bg: '#16161a',           // Pure Black Background
    card: '#262629',         // Very Dark Grey for Cards
    border: '#27272A',       // Subtle Border
    primary: '#F97316',      // Brand Orange
    textMain: '#FFFFFF',     // White Text
    textMuted: '#A1A1AA',    // Light Gray Text
};

// --- Vibrant Colors for Techniques & Equipment ---
const badgeStyles = {
    // === Techniques ===
    'Super Set': { bg: 'rgba(192, 132, 252, 0.15)', text: '#E9D5FF', border: '#A855F7' }, // Purple
    'Drop Set':  { bg: 'rgba(244, 63, 94, 0.15)',   text: '#FECDD3', border: '#F43F5E' }, // Rose
    'Pyramid':   { bg: 'rgba(251, 191, 36, 0.15)',  text: '#FDE68A', border: '#F59E0B' }, // Amber
    'Negative':  { bg: 'rgba(59, 130, 246, 0.15)',  text: '#BFDBFE', border: '#3B82F6' }, // Blue
    'Regular':   { bg: '#27272A',                   text: '#D4D4D8', border: '#52525B' }, // Zinc

    // === Equipment ===
    'Dumbbell':   { bg: 'rgba(34, 211, 238, 0.15)', text: '#A5F3FC', border: '#06B6D4' }, // Cyan
    'Barbell':    { bg: 'rgba(148, 163, 184, 0.15)', text: '#CBD5E1', border: '#64748B' }, // Slate
    'Machine':    { bg: 'rgba(167, 139, 250, 0.15)', text: '#DDD6FE', border: '#8B5CF6' }, // Violet
    'Cable':      { bg: 'rgba(250, 204, 21, 0.15)',  text: '#FEF08A', border: '#EAB308' }, // Yellow
    'Bodyweight': { bg: 'rgba(52, 211, 153, 0.15)',  text: '#A7F3D0', border: '#10B981' }, // Emerald
    'Smith Machine': { bg: 'rgba(244, 114, 182, 0.15)', text: '#FBCFE8', border: '#EC4899' }, // Pink
};
...
const styles = StyleSheet.create({
    ...
    tableHeaderRow: {
        flexDirection: 'row',
        backgroundColor: '#09090B', // Darker than card
        paddingVertical: 8,
...
// Helper to get colors (fuzzy match)
const getStyle = (text) => {
    if (!text) return badgeStyles['Regular'];
    const key = Object.keys(badgeStyles).find(k => text.toLowerCase().includes(k.toLowerCase()));
    return badgeStyles[key] || { bg: 'transparent', text: theme.textMuted, border: 'transparent' };
};

// Fallback for technique specific default
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.headerContainer}>
                    ...
                    <View style={styles.logoSection}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <View style={{ width: 3, height: 30, backgroundColor: theme.primary, marginRight: 10 }} />
                ...
                <View style={styles.sessionCard}>
                    <View style={styles.sessionBar} /> {/* Orange Bar Left */}
                    <View style={styles.sessionContent}>
                        <Text style={styles.sessionTitle}>{sessionName || `Day ${sessionNumber}`}</Text>
                        <Text style={styles.sessionMeta}>
                            Session #{sessionNumber}  •  {exercises.length} Exercises
                        </Text>
                    </View>
                </View>
                            ...
                            return (
                                <View key={sIdx} style={styles.tableRow}>
                                    ...
                                    <View style={styles.colIndex}>
                                        <Text style={[styles.cellText, { color: theme.textMuted }]}>{sIdx + 1}</Text>
                                    </View>
                                    ...
                                    {/* Weight */}
                                    <View style={[styles.colWeight, { flexDirection: 'row', justifyContent: 'center', alignItems: 'baseline' }]}>
                                    ...
                                    <View style={styles.colTech}>
                                        <View style={[styles.badgeContainer, { 
                                            backgroundColor: techSt.bg, 
                                            borderColor: techSt.border 
                                        }]}>
                                            <Text style={[styles.badgeText, { color: techSt.text }]}>
                                                {set.technique || 'Regular'}
                                            </Text>
                                        </View>
                                    </View>

                                    {/* Equipment (Colored or Text) */}
                                    <View style={styles.colEquip}>
                                        {hasEquipColor ? (
                                            <View style={[styles.badgeContainer, { 
                                                backgroundColor: equipSt.bg, 
                                                borderColor: equipSt.border 
                                            }]}>
                                                <Text style={[styles.badgeText, { color: equipSt.text }]}>
                                                    {safeText(set.equipment)}
                                                </Text>
                                            </View>
                                        ) : (
                                            <Text style={[styles.cellText, { color: theme.textMuted, fontSize: 9 }]}>
                                                {safeText(set.equipment)}
                                            </Text>
                                        )}
                                    </View>
