        ...
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-4">
                {myClients.map(sub => {
                    ...
                    return (
                        <div key={sub.id} className="bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800 p-5 rounded-2xl relative group overflow-hidden hover:border-zinc-300 dark:hover:border-zinc-700 transition-all shadow-sm">
                             <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${isOwner ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400' : 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-500'}`}>
                                        {sub.client_name ? sub.client_name.charAt(0) : 'C'}
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-zinc-900 dark:text-white leading-tight">{sub.client_name}</h4>
                                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${isOwner ? 'bg-zinc-100 dark:bg-zinc-900 text-zinc-500' : 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-500'}`}>
                            ...
                            <div className="space-y-2 mb-4 bg-zinc-50 dark:bg-zinc-900/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800/50">
                                ...
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="text-zinc-700 dark:text-zinc-300 font-bold">{sub.sessions_used} / {sub.plan_total_sessions}</span>
                                </div>
                            </div>
        ...
        return (
            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4">
                {transferHistory.map(req => {
                    ...
                    const cardBaseClass = "relative p-4 rounded-2xl border transition-all cursor-pointer hover:-translate-y-0.5 shadow-sm";
                    const cardColorClass = isIncoming 
                        ? "bg-white dark:bg-[#121214] border-l-4 border-l-blue-500 border-y-zinc-200 dark:border-y-zinc-800 border-r-zinc-200 dark:border-r-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900" // Incoming style
                        : "bg-zinc-50 dark:bg-zinc-900/40 border-l-4 border-l-zinc-400 dark:border-l-zinc-500 border-y-zinc-200 dark:border-y-zinc-800/50 border-r-zinc-200 dark:border-r-zinc-800/50 hover:bg-zinc-100 dark:hover:bg-zinc-900/60"; // Sent style
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex items-start gap-4">
                                    {/* Icon Box */}
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isIncoming ? 'bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-500' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-500'}`}>
                                        {isIncoming ? <ArrowDownLeft size={20} /> : <ArrowUpRight size={20} />}
                                    </div>

                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={`text-[10px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded ${isIncoming ? 'bg-blue-100 dark:bg-blue-500 text-blue-700 dark:text-black' : 'bg-zinc-200 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300'}`}>
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    <div className={`px-3 py-1 rounded-lg text-xs font-bold uppercase
                                        ${req.status === 'pending' ? 'bg-yellow-100 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-500' : ''}
                                        ${req.status === 'accepted' ? 'bg-green-100 dark:bg-green-500/10 text-green-600 dark:text-green-500' : ''}
                                        ${req.status === 'rejected' ? 'bg-red-100 dark:bg-red-500/10 text-red-600 dark:text-red-500' : ''}
                                    `}>
                                        {req.status}
                                    </div>
                                    ...
                                    {isIncoming && req.status === 'pending' && (
                                        <div className="flex gap-1 mt-1">
                                            <button 
                                                ...
                                                className="p-1.5 rounded-lg bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-500 hover:bg-green-500 hover:text-white transition-colors"
                                                title="Accept"
                                            >
                                            ...
                                            <button 
                                                ...
                                                className="p-1.5 rounded-lg bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-500 hover:bg-red-500 hover:text-white transition-colors"
                                                title="Reject"
                                            >
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white p-6 lg:p-10 lg:pl-80 pt-24 lg:pt-10 transition-colors duration-300">
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex gap-2 mb-8 border-b border-zinc-200 dark:border-zinc-800 pb-1">
                    <button 
                        ...
                        className={`px-4 py-2 text-sm font-bold border-b-2 transition-all ${activeTab === 'clients' ? 'border-orange-500 text-zinc-900 dark:text-white' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
                    >
                        My Clients
                    </button>
                    <button 
                        onClick={() => setActiveTab('history')} 
                        className={`px-4 py-2 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeTab === 'history' ? 'border-orange-500 text-zinc-900 dark:text-white' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
                    >
                        Transfer History
                    </button>
                </div>
            ...
            {isTransferModalOpen && (
                <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800 w-full max-w-lg rounded-3xl p-6 relative animate-in zoom-in-95 shadow-2xl">
                        <button onClick={() => setIsTransferModalOpen(false)} className="absolute top-6 right-6 text-zinc-400 hover:text-zinc-900 dark:hover:text-white"><XCircle size={24}/></button>
                        
                        <h2 className="text-xl font-bold text-zinc-900 dark:text-white mb-1">Transfer Sessions</h2>
                        <p className="text-zinc-500 text-sm mb-6">Request coverage for <span className="text-zinc-900 dark:text-white font-bold">{selectedSubForTransfer?.client_name}</span>.</p>
            ...
            {selectedRequestDetail && (
                <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800 w-full max-w-md rounded-3xl p-8 relative animate-in zoom-in-95 shadow-2xl">
                        <button onClick={() => setSelectedRequestDetail(null)} className="absolute top-6 right-6 text-zinc-400 hover:text-zinc-900 dark:hover:text-white"><XCircle size={24}/></button>

                        <div className="mb-6">
                            <span className="text-[10px] font-black uppercase tracking-wider text-zinc-500 bg-zinc-100 dark:bg-zinc-900 px-2 py-1 rounded">Transfer Details</span>
                            <h2 className="text-2xl font-black text-zinc-900 dark:text-white mt-2 mb-1">{selectedRequestDetail.client_name}</h2>
                        ...
                        <div className="space-y-4 bg-zinc-50 dark:bg-zinc-900/30 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800/50">
                            <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-800 pb-3">
                                ...
                                <span className="text-zinc-900 dark:text-white font-bold">{selectedRequestDetail.from_trainer_name}</span>
                            </div>
                            <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-800 pb-3">
                                <span className="text-zinc-500 font-bold text-xs uppercase">To</span>
                                <span className="text-zinc-900 dark:text-white font-bold">{selectedRequestDetail.to_trainer_name}</span>
                            </div>
                            <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-800 pb-3">
                                <span className="text-zinc-500 font-bold text-xs uppercase">Count</span>
                                <span className="text-zinc-900 dark:text-white font-bold">{selectedRequestDetail.sessions_count} Sessions</span>
                            </div>
                            <div>
                        ...
                        {selectedRequestDetail.status === 'pending' && selectedRequestDetail.to_trainer == (user.user_id || user.id) && (
                            <div className="grid grid-cols-2 gap-4 mt-6">
                                <button 
                                    ...
                                    className="py-3 rounded-xl border border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/50 font-bold"
                                >
                                    Reject
                                </button>
                                <button 
                        ...
                        {selectedRequestDetail.status !== 'pending' && (
                            <div className="mt-6 text-center p-3 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                                ...
                                <div className={`text-lg font-black uppercase mt-1 ${
                                    selectedRequestDetail.status === 'accepted' ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'
                                }`}>
