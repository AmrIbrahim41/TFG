...
// --- HELPER: NUTRITION CALCULATOR (نفس الكود الحسابي لم يتغير) ---
...
// --- HELPER: MODERN INPUT COMPONENT (UPDATED FOR THEME) ---
const ModernInput = ({ label, value, onChange, type="text", suffix, options, disabled=false, className="", min, placeholder }) => (
    <div className={`bg-zinc-200 dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 p-3 rounded-2xl relative focus-within:ring-2 focus-within:ring-orange-500/50 focus-within:border-orange-500 transition-all ${disabled ? 'opacity-50' : ''} ${className}`}>
        <label className="text-[10px] uppercase font-bold text-zinc-600 dark:text-zinc-500 mb-1 block">{label}</label>
        {options ? (
            <select disabled={disabled} value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-transparent text-zinc-900 dark:text-white font-bold text-sm outline-none appearance-none cursor-pointer">
                {options.map(o => <option key={o.val} value={o.val} className="dark:bg-zinc-900 bg-zinc-100">{o.lbl}</option>)}
            </select>
        ) : (
            <input 
                disabled={disabled} 
                ...
                className="w-full bg-transparent text-zinc-900 dark:text-white font-bold text-lg outline-none placeholder-zinc-400 dark:placeholder-zinc-600" 
            />
        )}
...
const ManualNutritionPlan = () => {
    ...
    if (!results) return <div className="p-10 text-zinc-900 dark:text-white">Loading Calculator...</div>;
        ...
        <div className="animate-in slide-in-from-bottom-4 duration-500 pb-20 p-6 pt-20 lg:pt-6 max-w-7xl mx-auto relative bg-zinc-50 dark:bg-[#09090b] min-h-screen transition-colors">
            ...
            {showHistory && (
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHistory(false)}>
                    <div className="bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800 rounded-3xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        
                        <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-[#18181b]/50 space-y-4">
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={() => setShowHistory(false)} className="p-2 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white rounded-full transition-colors"><X size={18}/></button>
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-zinc-100 dark:bg-black/50 border border-zinc-300 dark:border-zinc-700 focus:border-purple-500/50 rounded-xl pl-10 pr-4 py-3 text-sm text-zinc-900 dark:text-white placeholder-zinc-500 dark:placeholder-zinc-600 outline-none transition-all"
                                />
                            </div>
                        </div>

                        <div className="overflow-y-auto p-4 space-y-3 custom-scrollbar bg-zinc-100 dark:bg-[#0c0c0e]">
                            {savedPlans.filter(p => 
                                ...
                                savedPlans.filter(p => 
                                    ...
                                    <div 
                                        ...
                                        className="group relative bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 hover:border-purple-500/50 hover:bg-zinc-50 dark:hover:bg-[#1f1f22] rounded-2xl p-4 cursor-pointer transition-all duration-300 overflow-hidden shadow-sm"
                                    >
                                        ...
                                        <div className="flex justify-between items-start relative z-10">
                                            <div className="flex items-start gap-4">
                                                <div className="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-400 dark:text-zinc-500 font-black text-lg shadow-inner group-hover:text-purple-500 group-hover:border-purple-500/30 transition-colors">
                                            ...
                                            <button 
                                                ...
                                                className="p-2 text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0"
                                                title="Delete Plan"
                                            >
                                                <Trash2 size={18}/>
                                            </button>
                                        ...
                                        <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800/50 flex items-center gap-6 text-xs text-zinc-500 relative z-10 font-medium">
                                            <div className="flex items-center gap-1.5">
                                                <Smartphone size={14} className="text-zinc-400 dark:text-zinc-600"/>
            ...
            <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={handleNew} className="p-3 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-700 dark:text-white rounded-xl border border-zinc-300 dark:border-zinc-700 transition-all shadow-sm"><FileText size={18}/></button>
                    <button onClick={fetchHistory} className="p-3 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-700 dark:text-white rounded-xl border border-zinc-300 dark:border-zinc-700 transition-all shadow-sm"><FolderOpen size={18}/></button>
                    ...
                    <PDFDownloadLink document={<NutritionPDF_EN plan={dummyPlanObject} clientName={calcState.manualClientName} trainerName={calcState.manualTrainerName} brandText={calcState.brandText} carbAdjustment={calcState.carbAdjustment} results={results} exchangeList={exchangeList} notes={planNotes} />} fileName={`${calcState.manualClientName}_EN.pdf`}>
                        {({ loading }) => (<button disabled={loading} className="px-4 py-3 bg-zinc-200 dark:bg-white text-black font-bold rounded-xl flex items-center gap-2 shadow-sm">{loading ? <Loader2 size={18} className="animate-spin"/> : "EN"}</button>)}
                    </PDFDownloadLink>
                    <PDFDownloadLink document={<NutritionPDF_AR plan={dummyPlanObject} clientName={calcState.manualClientName} trainerName={calcState.manualTrainerName} brandText={calcState.brandText} carbAdjustment={calcState.carbAdjustment} results={results} exchangeList={exchangeList} notes={planNotes} />} fileName={`${calcState.manualClientName}_AR.pdf`}>
                        {({ loading }) => (<button disabled={loading} className="px-4 py-3 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400 font-bold rounded-xl border border-emerald-200 dark:border-emerald-800 flex items-center gap-2 shadow-sm">{loading ? <Loader2 size={18} className="animate-spin"/> : "AR"}</button>)}
                    </PDFDownloadLink>
                </div>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-[#121214] border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 relative overflow-hidden shadow-sm">
                 ...
                 {/* 1. CLIENT DATA */}
                <div className="bg-white dark:bg-[#121214] border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
                    <h3 className="font-bold text-zinc-800 dark:text-white mb-4 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        <div className="bg-zinc-200 dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 p-3 rounded-2xl flex flex-col justify-center"><label className="text-[10px] uppercase font-bold text-zinc-600 dark:text-zinc-500 flex items-center gap-1"><Scale size={10} /> In Lbs</label><span className="text-zinc-900 dark:text-white font-bold text-lg">{weightLbs}</span></div>
                ...
                {/* 2. STRATEGY */}
                <div className="bg-white dark:bg-[#121214] border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
                    <h3 className="font-bold text-zinc-800 dark:text-white mb-4 flex items-center gap-2"><Activity size={18} className="text-emerald-500"/> Strategy</h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                     <div className="lg:col-span-2 bg-zinc-200 dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                        <div className="text-center md:text-left min-w-[150px]">
                            ...
                            <p className="text-5xl font-black text-zinc-900 dark:text-white">{results.targetCalories} <span className="text-lg text-zinc-500 font-bold">kcal</span></p>
                        </div>
                        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                            <div className="bg-white dark:bg-zinc-950 p-3 rounded-2xl border border-zinc-300 dark:border-zinc-800/50"><span className="text-zinc-600 dark:text-zinc-400 flex items-center gap-2 text-[10px] uppercase font-bold mb-1"><Zap size={12} className="text-red-500"/> Protein</span><span className="text-zinc-900 dark:text-white font-black text-xl">{results.macros.protein.grams}g</span></div>
                            <div className="bg-white dark:bg-zinc-950 p-3 rounded-2xl border border-zinc-300 dark:border-zinc-800/50"><span className="text-zinc-600 dark:text-zinc-400 flex items-center gap-2 text-[10px] uppercase font-bold mb-1"><Wheat size={12} className="text-blue-500"/> Carbs</span><span className="text-zinc-900 dark:text-white font-black text-xl">{results.macros.carbs.grams}g</span></div>
                            <div className="bg-white dark:bg-zinc-950 p-3 rounded-2xl border border-zinc-300 dark:border-zinc-800/50"><span className="text-zinc-600 dark:text-zinc-400 flex items-center gap-2 text-[10px] uppercase font-bold mb-1"><Droplets size={12} className="text-yellow-500"/> Fats</span><span className="text-zinc-900 dark:text-white font-black text-xl">{results.macros.fats.grams}g</span></div>
                            <div className="bg-white dark:bg-zinc-950 p-3 rounded-2xl border border-zinc-300 dark:border-zinc-800/50"><span className="text-zinc-600 dark:text-zinc-400 flex items-center gap-2 text-[10px] uppercase font-bold mb-1"><Leaf size={12} className="text-emerald-500"/> Fiber</span><span className="text-zinc-900 dark:text-white font-black text-xl">{results.macros.fiber.grams}g</span></div>
                        </div>
                    </div>
                     <div className="bg-white dark:bg-[#121214] border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
                        <h3 className="font-bold text-zinc-800 dark:text-white mb-4 flex items-center gap-2"><FileText size={18} className="text-purple-500"/> PDF Brand</h3>
                ...
                <div className="space-y-6">
                    {exchangeList && Object.entries(exchangeList).map(([groupName, data]) => (
                        <div key={groupName} className="bg-white dark:bg-[#121214] border border-zinc-300 dark:border-zinc-800 rounded-3xl overflow-hidden shadow-sm">
                            <div className={`p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center ${data.bg}`}>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="text-xs font-bold text-zinc-700 dark:text-white bg-white/50 dark:bg-black/20 px-3 py-1 rounded-lg border border-white/20">Target: {Math.round(data.targetCals)} kcal / meal</div>
                                </div>
                            </div>
                            <div className="p-0">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 divide-y divide-zinc-200 dark:divide-zinc-800 md:divide-y-0 md:gap-px bg-zinc-200 dark:bg-zinc-800/50 border-zinc-200">
                                    {data.items.length > 0 ? (data.items.map((item, idx) => (
                                        <div key={idx} className="bg-white dark:bg-[#121214] p-4 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors flex justify-between items-center">
                                            <span className="font-bold text-zinc-700 dark:text-zinc-300">{item.name}</span>
                                            <div className="text-right"><div><span className={`font-black text-lg ${groupName === 'Carbohydrates' && calcState.carbAdjustment !== 0 ? 'text-blue-500 dark:text-blue-400' : 'text-zinc-900 dark:text-white'}`}>{item.weight}</span><span className="text-zinc-500 text-xs font-bold ml-1">{item.unit}</span></div><div className="text-[10px] text-zinc-500 font-medium">~ {item.meta.cals} kcal</div></div>
                                        </div>
                                    ))) : (<div className="col-span-full p-8 text-center text-zinc-500 text-sm bg-white dark:bg-[#121214]">No items found for this category in database.</div>)}
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                {/* 5. NOTES */}
                <div className="bg-white dark:bg-[#121214] border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
                    <h3 className="font-bold text-zinc-800 dark:text-white mb-4 flex items-center gap-2"><FileText size={18} className="text-emerald-500"/> Coach Notes & Recommendations</h3>
                    <textarea value={planNotes} onChange={(e) => setPlanNotes(e.target.value)} placeholder="Write your grocery list, supplement recommendations, or snack ideas here..." className="w-full bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-2xl p-4 text-zinc-800 dark:text-zinc-300 font-medium outline-none focus:border-emerald-500/50 transition-colors resize-none text-sm leading-relaxed min-h-[150px] placeholder-zinc-400 dark:placeholder-zinc-600"></textarea>
                </div>
            </div>
        </div>
    );
