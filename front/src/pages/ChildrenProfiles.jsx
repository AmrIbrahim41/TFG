        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                ...
                <button 
                    ...
                    className="group bg-white text-black hover:bg-blue-500 hover:text-white font-bold py-4 px-8 rounded-2xl flex items-center gap-3 transition-all shadow-xl shadow-white/5 hover:shadow-blue-500/20 active:scale-95 whitespace-nowrap"
                >
            ...
            {loading ? (
                ...
                <>
                            ...
                            >
                                ...
                                <div className="relative z-10 flex items-start gap-5">
                                    <div className="relative">
                                        <div className="w-20 h-20 rounded-2xl bg-zinc-900 overflow-hidden border-2 border-zinc-800 group-hover:border-blue-500/50 transition-colors shadow-lg">
                                            {child.photo_url ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center text-zinc-600 bg-zinc-800"><User size={28} /></div>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex-1 space-y-1 pt-1">
                                        <h3 className="text-xl font-bold text-white group-hover:text-blue-400 transition-colors truncate">{child.name}</h3>
                                        <div className="flex items-center gap-2">
                                            <span className="bg-zinc-900 text-zinc-400 text-xs font-mono px-2 py-1 rounded-lg border border-zinc-800 flex items-center gap-1"><Hash size={10} /> {child.manual_id}</span>
                                            {child.age && <span className="bg-blue-500/10 text-blue-400 text-xs font-bold px-2 py-1 rounded-lg border border-blue-500/20">{child.age} Years</span>}
                                        </div>
                                    </div>
                                    <div className="h-full flex items-center justify-center">
                                        <div className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center text-zinc-500 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300"><ArrowRight size={18} /></div>
                                    </div>
                                </div>
                                <div className="mt-6 pt-4 border-t border-zinc-800/50 flex items-center justify-between text-xs font-medium text-zinc-500">
                                    ...
                                    <span className={child.is_subscribed ? "text-green-500" : "text-zinc-600"}>{child.is_subscribed ? "Active Member" : "No Active Plan"}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex justify-center items-center gap-4 mt-12 mb-4">
                        <button onClick={() => fetchChildren(prevPage)} disabled={!prevPage} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 font-bold hover:bg-zinc-800 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronLeft size={18} /> Previous</button>
                        <button onClick={() => fetchChildren(nextPage)} disabled={!nextPage} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 font-bold hover:bg-zinc-800 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all">Next <ChevronRight size={18} /></button>
                    </div>
                </>
            )}
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-[#121214] border border-zinc-800 w-full max-w-md rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-200 relative overflow-hidden">
                        <div className="relative z-10 mb-6">
                            <h2 className="text-2xl font-black text-white flex items-center gap-2"><Sparkles className="text-blue-500" size={24} /> New Child Profile</h2>
                        ...
                        <form onSubmit={handleAddChild} className="space-y-5 relative z-10">
                            <div>
                                ...
                                <input required className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white outline-none focus:border-blue-500 transition-all" placeholder="e.g. Aly Ahmed" value={newChild.name} onChange={e => setNewChild({...newChild, name: e.target.value})} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-zinc-500 uppercase ml-1 mb-1.5 block">Manual ID</label>
                                    <input required className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white outline-none focus:border-blue-500 transition-all font-mono" placeholder="1024" value={newChild.manual_id} onChange={e => setNewChild({...newChild, manual_id: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-zinc-500 uppercase ml-1 mb-1.5 block">Parent Phone</label>
                                    <input required className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white outline-none focus:border-blue-500 transition-all" placeholder="010..." value={newChild.phone} onChange={e => setNewChild({...newChild, phone: e.target.value})} />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1 mb-1.5 block">Photo (Optional)</label>
                                <input type="file" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-400 file:bg-zinc-800 file:text-zinc-300 file:border-0 hover:file:bg-zinc-700 transition-all" onChange={e => setNewChild({...newChild, photo: e.target.files[0]})} />
                            </div>
