...
import {
  ...
  Save, FolderOpen, History, Smartphone, Search, FilePlus, Calendar,
  Briefcase, MessageSquare, Loader2
} from 'lucide-react';
import api from '../api';
...
const ManualTrainingPlan = () => {
  ...
  // Debounced values for PDF rendering
  ...
  const [pdfData, setPdfData] = useState({ exercises, identity: { clientName, trainerName, sessionName, brandName } });

  useEffect(() => {
    setPdfData({ exercises: debouncedExercises, identity: debouncedIdentity });
  ...
  const filteredHistory = useMemo(
    () =>
      savedWorkouts.filter((w) =>
        ...
        (w.client_name || '').toLowerCase().includes(historySearch.toLowerCase())
      ),
    [savedWorkouts, historySearch]
  );
  ...
  const handleExerciseCount = useCallback((delta) => {
    setExercises((prev) => {
      const newEx = [...prev];
      if (delta > 0) {
        newEx.push(EMPTY_EXERCISE());
      } else {
        if (newEx.length > 1) {
          if (newEx[newEx.length - 1].name.trim() !== '' && !window.confirm('Remove last exercise?')) return prev;
          newEx.pop();
        } else {
  ...
  const removeExercise = useCallback((idx) => {
    setExercises((prev) => {
      if (prev.length <= 1) { toast.error('Minimum 1 exercise required'); return prev; }
  ...
  const handleNewPlan = useCallback(() => {
    setCurrentId(null);
    setClientName(''); setPhone(''); setSessionName(''); setTrainerName('');
  ...
  const handleSave = useCallback(async () => {
    if (!sessionName.trim()) {
      toast.error('Session name is required before saving.');
      return;
    }
  ...
  }, [sessionName, clientName, phone, exercises, trainerName, brandName, currentId]);
  ...
  const handleLoad = useCallback((w) => {
    ...
    setPdfData({ exercises: ne, identity: { clientName: nc, trainerName: nt, sessionName: ns, brandName: nb } });
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#09090B] text-zinc-900 dark:text-zinc-100 flex flex-col relative transition-all duration-300 pt-16 lg:pt-0">
      ...
      {showHistory && (
        ...
        >
          <div
            className="bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800 rounded-3xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex flex-col gap-3">
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                  <History size={18} /> Saved Workouts
                </h3>
                <button
                  onClick={() => setShowHistory(false)}
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by Session or Client Name..."
                  value={historySearch}
            ...
            <div className="overflow-y-auto p-4 space-y-2 custom-scrollbar">
              {filteredHistory.length === 0 ? (
                <p className="text-zinc-500 text-center py-10">No saved workouts found.</p>
              ) : (
                filteredHistory.map((w) => (
                  <div
                    key={w.id}
                    onClick={() => handleLoad(w)}
                    className="p-4 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-orange-500 cursor-pointer flex justify-between items-center group transition-all"
                  >
                    <div>
                      <h4 className="font-bold text-zinc-900 dark:text-white text-lg">
                        {w.session_name || 'Untitled Workout'}
                      </h4>
                      <span className="text-xs text-zinc-500">{w.client_name}</span>
                    </div>
                    <button
                      onClick={(e) => handleDelete(w.id, e)}
                      className="p-2 text-zinc-500 dark:text-zinc-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
      ...
      <div className="sticky top-16 lg:top-0 z-40 bg-zinc-50/95 dark:bg-[#09090B]/95 backdrop-blur-xl border-b border-zinc-200 dark:border-zinc-800 transition-colors">
        <div className="px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-lg md:text-xl font-black text-zinc-900 dark:text-white flex items-center gap-2">
            <Dumbbell className="text-orange-600 dark:text-orange-500" size={24} />
            <span className="hidden sm:inline">Manual Training Plan</span>
          </h1>
          <div className="flex items-center gap-2">
            <button
              onClick={handleNewPlan}
              className="p-2 bg-zinc-200 dark:bg-zinc-800 rounded-lg text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white"
              title="New Plan"
            >
              <FilePlus size={18} />
            </button>
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-lg text-sm flex items-center gap-2 shadow-sm disabled:opacity-70 transition-all"
            >
        ...
        {showIdentity && (
          <div className="px-3 pb-3 pt-1 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30">
            <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { icon: Activity, val: sessionName, set: setSessionName, ph: 'Session Name*' },
                { icon: User, val: clientName, set: setClientName, ph: 'Client Name' },
                { icon: Smartphone, val: phone, set: setPhone, ph: 'Phone Number' },
                { icon: Briefcase, val: trainerName, set: setTrainerName, ph: 'Trainer Name' },
              ].map(({ icon: Icon, val, set, ph }) => (
                <div key={ph} className="relative group">
                  <Icon size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-orange-500 transition-colors" />
                  <input
                    value={val}
                    onChange={(e) => set(e.target.value)}
                    placeholder={ph}
                    className="w-full bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-9 pr-3 py-2.5 text-sm font-medium text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20 transition-all"
                  />
                </div>
              ))}
            </div>
        ...
        <div className="px-4 py-1 flex justify-center">
          <button
            ...
            className="text-[10px] font-bold text-zinc-400 hover:text-orange-500 uppercase tracking-widest flex items-center gap-1 transition-colors"
          >
            <ArrowDown size={10} className={showIdentity ? 'rotate-180' : ''} />
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4 pb-32 custom-scrollbar">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          {/* Floating counter */}
          <div className="flex justify-end items-center sticky top-0 z-10 py-2 -my-2 pointer-events-none">
            <div className="flex items-center gap-1 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm rounded-full p-1 border border-zinc-200 dark:border-zinc-800 shadow-sm pointer-events-auto">
              <button onClick={() => handleExerciseCount(-1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all active:scale-90">
                <Minus size={14} />
              </button>
              <span className="min-w-[90px] text-center text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">
                {exercises.length} Exercises
              </span>
              <button onClick={() => handleExerciseCount(1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-orange-100 dark:bg-orange-500/10 text-orange-600 dark:text-orange-500 hover:bg-orange-200 dark:hover:bg-orange-500/20 transition-all active:scale-90">
                <Plus size={14} />
              </button>
            </div>
          </div>
          ...
          {exercises.map((ex, exIndex) => (
            <div key={exIndex} className="group relative bg-white dark:bg-[#121214] border border-zinc-200 dark:border-white/5 rounded-3xl p-1 shadow-sm transition-all hover:shadow-lg">
              <div className="p-3 md:p-6 pb-2 flex items-start gap-3">
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent text-lg md:text-2xl font-bold text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-700 outline-none"
                    />
                    <button onClick={() => removeExercise(exIndex)} className="md:hidden p-2 text-zinc-400 hover:text-red-500">
                      <Trash2 size={16} />
                    </button>
                  </div>
                  <div className="flex items-center justify-between pl-1">
                    ...
                    <div className="flex items-center gap-1 bg-zinc-50 dark:bg-zinc-950 rounded-lg p-1 border border-zinc-200 dark:border-zinc-800/50 ml-auto md:ml-0">
                      <button onClick={() => handleSetCount(exIndex, -1)} className="w-7 h-7 flex items-center justify-center rounded bg-zinc-200 dark:bg-zinc-900 text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-300 dark:hover:bg-zinc-800 transition-all active:scale-90"><Minus size={12} /></button>
                ...
                <button onClick={() => removeExercise(exIndex)} className="hidden md:block p-2 text-zinc-400 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100">
                  <Trash2 size={18} />
                </button>
              </div>
                    ...
                    <div key={setIndex} className="relative bg-white dark:bg-zinc-900/60 rounded-xl p-2 md:p-0 md:bg-transparent md:hover:bg-zinc-200 dark:md:hover:bg-zinc-900/30 transition-colors grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-3 md:items-center border border-zinc-200 dark:border-transparent md:border-0 shadow-sm md:shadow-none">
                      <div className="flex md:contents flex-wrap gap-2">
                        <div className="flex items-center gap-2 w-full md:w-auto md:contents">
                          ...
                          {ex.sets.length > 1 ? (
                            <button onClick={() => removeSet(exIndex, setIndex)} className="md:hidden w-8 h-8 flex items-center justify-center text-zinc-400 hover:text-red-500 bg-zinc-100 dark:bg-zinc-800 rounded-lg ml-1"><X size={16} /></button>
                          ) : (
              ...
              <div className="mt-2 px-2 pb-2">
                <button onClick={() => setActiveNoteIndex((n) => (n === exIndex ? null : exIndex))} className="flex items-center gap-2 text-xs font-bold text-zinc-400 hover:text-orange-500 transition-colors py-1">
                  <MessageSquare size={14} />
                  {ex.note ? 'Edit Note' : 'Add Note'}
                </button>
                {(activeNoteIndex === exIndex || ex.note) && (
                  <div className={`mt-1 overflow-hidden transition-all ${activeNoteIndex === exIndex ? 'max-h-32 opacity-100' : ex.note ? 'max-h-24 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <textarea
                      value={ex.note || ''}
        ...
        <div className="flex gap-3 bg-zinc-50/90 dark:bg-[#121214]/90 backdrop-blur-xl p-2 rounded-2xl border border-zinc-300 dark:border-white/10 shadow-2xl dark:shadow-black/80">
          {isClient && (
            ...
            >
              {({ loading: pdfLoading }) => (
                <>
                  {pdfLoading ? <Activity size={16} className="animate-spin" /> : <FileText size={16} />}
                  <span>{pdfLoading ? 'Generating...' : 'Download PDF (EN)'}</span>
                </>
              )}
            </PDFDownloadLink>
          )}
