...
const TECHNIQUE_CONFIG = { 'Regular': { color: 'text-zinc-400', bg: 'bg-zinc-800/50', border: 'border-zinc-700', icon: Activity }, 'Drop Set': { color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20', icon: ArrowDown }, 'Super Set': { color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20', icon: Layers }, 'Pyramid': { color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20', icon: TrendingUp }, 'Negative': { color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20', icon: Zap } };
const EQUIP_CONFIG = { 'Bodyweight': { color: 'text-emerald-400', icon: UserIcon }, 'Dumbbell': { color: 'text-blue-400', icon: Dumbbell }, 'Barbell': { color: 'text-zinc-300', icon: Grip }, 'Cable': { color: 'text-cyan-400', icon: Zap }, 'Machine': { color: 'text-indigo-400', icon: Settings } };
...
const ManualTrainingPlan = () => {
    ...
    return (
        // FIX: Added 'pt-16 lg:pt-0' to push content down on mobile (below fixed header)
        // and kept 'lg:pl-72' for desktop sidebar.
        <div className="min-h-screen bg-[#09090B] text-zinc-100 flex flex-col relative lg:pl-72 transition-all duration-300 pt-16 lg:pt-0">
            ...
            {showHistory && (
                <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHistory(false)}>
                    <div className="bg-[#121214] border border-zinc-800 rounded-3xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                        <div className="p-4 border-b border-zinc-800 flex flex-col gap-3">
                            <div className="flex justify-between items-center">
                                <h3 className="font-bold text-white flex items-center gap-2"><History size={18}/> Saved Workouts</h3>
                                <button onClick={() => setShowHistory(false)} className="p-2 hover:bg-zinc-800 rounded-full"><X size={18}/></button>
                            </div>
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-zinc-900 border border-zinc-800 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-orange-500/50"
                                />
                            </div>
                        </div>
                        <div className="overflow-y-auto p-4 space-y-2 custom-scrollbar">
                            ...
                            filteredHistory.map(w => (
                                <div key={w.id} onClick={() => handleLoad(w)} className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-orange-500 cursor-pointer flex justify-between items-center group transition-all">
                                    <div>
                                        <h4 className="font-bold text-white text-lg">{w.session_name || "Untitled Workout"}</h4>
                                        <span className="text-xs text-zinc-500">{w.client_name}</span>
                                    </div>
                                    <button onClick={(e) => handleDelete(w.id, e)} className="p-2 text-zinc-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"><Trash2 size={16}/></button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {/* FIX: Changed 'top-0' to 'top-16 lg:top-0' so it sticks BELOW the mobile header on phones */}
            <div className="sticky top-16 lg:top-0 z-40 bg-[#09090B]/95 backdrop-blur-xl border-b border-white/5">
                <div className="px-4 py-3 flex items-center justify-between">
                    ...
                    <h1 className="text-lg md:text-xl font-black text-white flex items-center gap-2">
                        <Dumbbell className="text-orange-500" size={24}/><span className="hidden sm:inline">Manual Training Plan</span>
                    </h1>
                    <div className="flex items-center gap-2">
                         <button onClick={handleNewPlan} className="p-2 bg-zinc-800 rounded-lg text-zinc-300 hover:text-white" title="New Plan"><FilePlus size={18}/></button>
                         <button onClick={fetchHistory} className="p-2 bg-zinc-800 rounded-lg text-zinc-300 hover:text-white" title="History"><FolderOpen size={18}/></button>
                         <button onClick={handleSave} className="px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-lg text-sm flex items-center gap-2"><Save size={16}/> {currentId ? 'Update' : 'Save'}</button>
                    </div>
                </div>
                {showIdentity && (
                    <div className="px-4 pb-4 pt-2 border-t border-white/5 bg-zinc-950/50">
                        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                            <input value={sessionName} onChange={(e) => setSessionName(e.target.value)} placeholder="Session Name..." className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-orange-500/50 transition-all" />
                            <input value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Client Name..." className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-orange-500/50 transition-all" />
                            <div className="relative">
                                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone (Internal)" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg pl-3 pr-8 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-orange-500/50 transition-all" />
                                <Smartphone size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-600"/>
                            </div>
                            <input value={trainerName} onChange={(e) => setTrainerName(e.target.value)} placeholder="Trainer Name..." className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-orange-500/50 transition-all" />
                        </div>
                    </div>
                )}
                <div className="px-4 pb-2">
                    <div className="max-w-4xl mx-auto flex justify-end">
                        <button onClick={() => setShowIdentity(!showIdentity)} className="text-xs font-bold text-zinc-500 hover:text-white flex items-center gap-1 bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800">
             ...
             <div className="flex-1 overflow-y-auto p-4 pb-32 custom-scrollbar">
                <div className="max-w-4xl mx-auto space-y-6">
                    <div className="flex items-center justify-between mb-2">
                        <h2 className="text-xl font-bold text-white flex items-center gap-2"><Dumbbell className="text-orange-500" size={20}/> Exercises</h2>
                    ...
                    {exercises.map((ex, exIndex) => (
                        <div key={exIndex} className="group relative bg-[#121214] border border-white/5 rounded-3xl p-1 shadow-2xl transition-all hover:border-white/10 animate-in fade-in duration-300">
                            <div className="p-4 md:p-6 pb-2 flex items-start gap-4">
                                <div className="hidden md:flex flex-col items-center justify-center w-12 h-12 rounded-2xl bg-zinc-900 border border-zinc-800 text-zinc-500 font-black text-xl shadow-inner">{String(exIndex + 1).padStart(2, '0')}</div>
                                <div className="flex-1 space-y-3">
                                    <div className="flex items-center gap-3">
                                        <span className="md:hidden text-lg font-black text-zinc-600">{String(exIndex + 1).padStart(2, '0')}</span>
                                        <input value={ex.name} onChange={(e) => updateExercise(exIndex, 'name', e.target.value)} placeholder="Exercise Name..." className="w-full bg-transparent text-xl md:text-2xl font-bold text-white placeholder-zinc-700 outline-none" />
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest hidden md:inline">{ex.sets.length} Sets Configured</span>
                                ...
                                <button onClick={() => { if(exercises.length > 1 && confirm('Delete exercise?')) setExercises(prev => prev.filter((_, i) => i !== exIndex)); }} className="p-2 text-zinc-700 hover:text-red-500 transition-all opacity-100 md:opacity-0 md:group-hover:opacity-100"><Trash2 size={18} /></button>
                            </div>
                                    ...
                                    return (
                                        <div key={setIndex} className="relative bg-zinc-900/40 rounded-xl p-3 md:p-0 md:bg-transparent md:hover:bg-zinc-900/30 transition-colors grid grid-cols-1 md:grid-cols-12 gap-3 md:items-center">
                                            ...
                                            <div className="flex gap-2 md:contents">
                                                <div className="flex-1 md:col-span-2"><input type="number" placeholder="0" value={set.reps} onChange={(e) => updateSet(exIndex, setIndex, 'reps', e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 focus:border-orange-500/50 rounded-lg py-3 md:py-2.5 text-center text-sm font-bold text-white outline-none transition-all placeholder:text-zinc-800 appearance-none"/></div>
                                                <div className="flex-1 md:col-span-2"><input type="number" placeholder="0" value={set.weight} onChange={(e) => updateSet(exIndex, setIndex, 'weight', e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 focus:border-orange-500/50 rounded-lg py-3 md:py-2.5 text-center text-sm font-bold text-white outline-none transition-all placeholder:text-zinc-800 appearance-none"/></div>
                                            </div>
                                            <div className="md:col-span-4 mt-1 md:mt-0"><div className={`flex items-center w-full rounded-lg px-3 py-0.5 border transition-all ${tech.bg} ${tech.border}`}><TechIcon size={14} className={`${tech.color} mr-2`} /><select value={set.technique} onChange={(e) => updateSet(exIndex, setIndex, 'technique', e.target.value)} className="w-full bg-transparent text-[11px] md:text-xs font-bold uppercase py-2.5 outline-none text-zinc-200 cursor-pointer">{Object.keys(TECHNIQUE_CONFIG).map(k => <option key={k} value={k} className="bg-zinc-900">{k}</option>)}</select></div></div>
                                            <div className="md:col-span-3 mt-1 md:mt-0"><div className="flex items-center w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-0.5 hover:border-zinc-700 transition-colors"><EquipIcon size={14} className={`${equip.color} mr-2`} /><select value={set.equipment} onChange={(e) => updateSet(exIndex, setIndex, 'equipment', e.target.value)} className="w-full bg-transparent text-[11px] md:text-xs font-bold text-zinc-300 py-2.5 outline-none cursor-pointer"><option value="" className="bg-zinc-900">No Equip</option>{Object.keys(EQUIP_CONFIG).map(k => <option key={k} value={k} className="bg-zinc-900">{k}</option>)}</select></div></div>
                                        </div>
                                    );
                                })}
                            </div>
                ...
                <div className="flex gap-3 bg-[#121214]/90 backdrop-blur-xl p-2 rounded-2xl border border-white/10 shadow-2xl shadow-black/80">
                    ...
                    {isClient && (
                         <PDFDownloadLink
                            ...
                            className="flex-1 py-3.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95"
                        >
                            {({ loading }) => (
