        ...
        <div className="min-h-screen bg-[#09090b] text-white p-6 pt-24 lg:pl-80 animate-in fade-in duration-500">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-zinc-800/60 pb-8">
                    ...
                    {/* Navigation Tabs */}
                    <div className="flex bg-zinc-900/80 p-1.5 rounded-2xl border border-zinc-800 backdrop-blur-sm">
                        <button 
                            onClick={() => setActiveTab('profiles')}
                            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'profiles' ? 'bg-zinc-800 text-white shadow-lg border border-zinc-700' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            <LayoutGrid size={16} /> Profiles
                        </button>
                        <button 
                            onClick={() => setActiveTab('schedule')}
                            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'schedule' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            <Calendar size={16} /> Schedule
                        </button>
                        <button 
                            onClick={() => setActiveTab('history')}
                            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'history' ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-500/30' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
