...
const WorkoutEditor = () => {
    ...
    // Client Info for Navigation & PDF
            ...
            try {
                if (subId) {
                    try {
                        ...
                        // Capture Client ID for the Back Button
    ...
    const handleBack = () => {
        if (clientId) {
            // Navigate explicitly to the client profile and request the 'training' tab
            // Assuming your Client Profile accepts state or query param for tabs.
            // Example: passing state { activeTab: 'training' } or similar
        ...
        <div className="fixed inset-0 z-[200] flex flex-col bg-[#09090b] text-white font-sans selection:bg-orange-500/30">
            ...
            <div className="shrink-0 z-50 bg-[#09090b]/95 backdrop-blur-xl border-b border-zinc-800/50 sticky top-0 safe-area-top shadow-md">
                <div className="max-w-4xl mx-auto px-3 h-16 grid grid-cols-[44px_1fr_auto] items-center gap-2">
                    <button onClick={handleBack} className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 flex items-center justify-center transition-colors"><ArrowLeft size={20} className="text-white" /></button>
                    <div className="flex flex-col items-center justify-center min-w-0 px-2">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-500 mb-0.5 truncate max-w-full">Session {sessionNum}</span>
                        <input value={sessionName || ''} onChange={(e) => setSessionName(e.target.value)} placeholder="Workout Name" className="bg-transparent text-center text-base md:text-lg font-bold text-white placeholder-zinc-700 outline-none w-full border-b border-transparent focus:border-zinc-700 transition-all pb-0.5 truncate" />
                    </div>
                    <div className="relative flex items-center gap-2" ref={menuRef}>
                        {/* History Button (Restored) */}
                        <button 
                            onClick={() => setShowHistory(true)} 
                            className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-orange-500 hover:border-orange-500/50 flex items-center justify-center transition-all"
                            title="View History"
                        >
                            <History size={18} />
                        </button>

                        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all ${isMenuOpen ? 'bg-orange-600 text-white border-orange-500' : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white'}`}><MoreVertical size={20} /></button>
                                ...
                                <div className="space-y-2">
                                    {isClient && (
                                        <PDFDownloadLink
                                            ...
                                            className="w-full px-4 py-3 rounded-xl bg-gradient-to-r from-zinc-800 to-zinc-700 hover:from-zinc-700 hover:to-zinc-600 text-white font-bold text-sm flex items-center justify-between gap-3 transition-all shadow-lg hover:shadow-xl active:scale-98"
                                        >
                                            {({ loading }) => (
                                                <><div className="flex items-center gap-3">{loading ? <Activity size={18} className="animate-spin"/> : <FileText size={18} className="text-orange-400"/>}<span>Download PDF (EN)</span></div><ChevronRight size={16} className="text-zinc-400"/></>
                                            )}
                                        </PDFDownloadLink>
                                    )}
                                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 pb-32 custom-scrollbar">
                <div className="max-w-4xl mx-auto space-y-6">
                     {exercises.map((ex, exIndex) => (
                        <div key={exIndex} className="group relative bg-[#121214] border border-white/5 rounded-3xl p-1 shadow-2xl transition-all hover:border-white/10">
                     ...
                     <div onClick={() => handleExerciseCount(1)} className="p-8 rounded-3xl border-2 border-dashed border-zinc-800 text-zinc-600 font-bold flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-orange-500 hover:text-orange-500 hover:bg-orange-500/5 transition-all">
                        <Plus size={32} />
            ...
            {/* HISTORY SIDE DRAWER */}
            {showHistory && (
                <div className="fixed inset-0 z-[250] bg-black/80 backdrop-blur-sm flex justify-end">
                    <div className="w-full max-w-md bg-[#121214] h-full border-l border-zinc-800 animate-in slide-in-from-right duration-300 flex flex-col shadow-2xl">
                        {/* Drawer Header */}
                        <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-[#121214]">
                            <h3 className="font-bold text-white flex items-center gap-2"><History size={18} className="text-orange-500"/> Workout History</h3>
                            <button onClick={() => setShowHistory(false)} className="w-8 h-8 rounded-full bg-zinc-900 flex items-center justify-center text-zinc-500 hover:text-white transition-colors"><X size={18} /></button>
                        </div>
                                        ...
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[10px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded border border-zinc-700">Session {session.session_number}</span>
                                            ...
                                            <button 
                                                ...
                                                className="text-[10px] font-bold bg-orange-500/10 text-orange-500 border border-orange-500/20 px-2 py-1 rounded hover:bg-orange-500 hover:text-white transition-all"
                                            >
                                                LOAD
                                            </button>
                                        </div>
                                        <div className="space-y-1">
                                            {session.exercises && session.exercises.slice(0, 3).map((ex, i) => (
                                                <div key={i} className="text-xs text-zinc-400 flex justify-between">
                                                    <span>• {ex.name}</span>
                                                    <span className="text-zinc-600">{ex.sets?.length} sets</span>
                                                </div>
                                            ))}
                                            {session.exercises && session.exercises.length > 3 && (
                                                <div className="text-[10px] text-zinc-600 italic pl-2">+ {session.exercises.length - 3} more exercises</div>
                                            )}
                                        </div>
                                    </div>
                                ))
